...
class oint_client(Node):
    ...
    def __init__(self):
        ...
        self.cli = self.create_client(RobotInterpolationZad5, 'oint_control_srv')
        while not self.cli.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('service not available, waiting again...')
        self.req = RobotInterpolationZad5.Request()
    ...
    def send_request(self):
        try:
            ...
            self.req.a= float(sys.argv[4])
            self.req.b= float(sys.argv[5])
            self.req.move_time = float(sys.argv[6])
            self.req.interpolation_method = sys.argv[7]
